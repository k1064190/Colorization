...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def image_generator():
        ...
        for image_file in cycle(image_files):  # cycle through the files indefinitely
            image_path = os.path.join(args.train_data_dir, image_file)
            try:
                image = Image.open(image_path).convert("RGB")
                yield {
                    "pixel_values": image_transforms(image),
                    "conditioning_pixel_values": conditioning_image_transforms(image),
                    "input_ids": tokenizer(
                        "",  # Empty string as caption
    ...
    train_dataset = IterableDataset.from_generator(image_generator)
